...
# expiriment - varaing lambda from 0 to 0.1 in 10 steps
...
epochrange = [50, 150]
train_acc_avg = np.mean(train_acc_results[epochrange[0]:epochrange[1],:noOFLambda], axis=0)
test_acc_avg = np.mean(test_acc_results[epochrange[0]:epochrange[1], :noOFLambda], axis=0)
plt.plot(L2Lambda[:noOFLambda], train_acc_avg, label="train accuracy", color="red", marker="o")
plt.plot(L2Lambda[:noOFLambda], test_acc_avg, label="test accuracy", color="blue", marker="o")
plt.xlabel("lambda") 
plt.ylabel("accuracy")
