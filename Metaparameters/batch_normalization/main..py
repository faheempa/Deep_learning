...
class ANN(nn.Module):
    ...
    def forward(self, x, do_batch_normalization=False):
        x = self.input(x)
        x = self.relu(x)
...
# train model
def train_model(model, optimizer, loss_fn, train_dataloader, test_dataloader, epochs=100, batchNorm=True):
...
train_acc_1, test_acc_1 = train_model(model, optimizer, loss_fn, train_dataloader, test_dataloader, epochs, batchNorm)

print("\ntraining model with batch normalization")
batchNorm = True
train_dataloader, test_dataloader = create_dataloaders(batch_size=32)
model, optimizer, loss_fn = create_model(input_dim=train_data.shape[1], output_dim=1)
train_acc_2, test_acc_2 = train_model(model, optimizer, loss_fn, train_dataloader, test_dataloader, epochs, batchNorm)

# plot results
plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
plt.plot(train_acc_1, label='without batch normalization')
plt.plot(train_acc_2, label='with batch normalization')
...
# plot results
plt.subplot(1, 2, 2)
plt.plot(test_acc_1, label='without batch normalization')
plt.plot(test_acc_2, label='with batch normalization')
